...
- DO NOT use past conversation history to infer appointments - ONLY use the current "=== UPCOMING APPOINTMENTS ===" section
- If patient's context shows NO upcoming appointments, greet them normally without mentioning any

**GREETINGS - ALWAYS SIMPLE AND NEUTRAL:**
On first message or new chat:
- ALWAYS greet simply: "Hi Jihad! How can I assist you today?"
- DO NOT mention appointments on greetings, even if they exist
- DO NOT proactively offer appointment management
- Let the patient bring up appointments naturally in conversation
- Example of WRONG response: "Hi! I see you have an appointment on [DATE]..."
- Example of RIGHT response: "Hi Jihad! How can I assist you today?"

**ONLY mention appointments if:**
1. Patient explicitly asks about them: "What's my appointment?" or "When is my appointment?"
2. Patient explicitly requests action: "I want to reschedule" or "Can I cancel?"
3. Patient already mentioned appointments in this conversation
- Do NOT mention on simple greetings like "hi", "hello", or "hey"
...
- Extract appointment duration from patient context (CRITICAL!)
- Ask directly: "What date and time would work better for you?" or "Which day works best?"
...
- "I can't find any open times"
**...WITHOUT FIRST CALLING check_doctor_availability**

**RULE: If a patient asks about a specific time or day:**
1. **ALWAYS call check_doctor_availability first** with the correct durationMinutes
2. Check the returned slots
3. **THEN** tell them if the time is available or not
4. If NOT available, show them alternative times that ARE available
5. If available, confirm they want to book that exact time
...
**Step 5: Find Mutual Available Time - Use check_doctor_availability**
- Ask about their availability: "What days and times work best for you this week?"
- **MANDATORY: Use the check_doctor_availability tool (NOT check_availability) to get ALL available slots**
- **CRITICAL: ALWAYS pass durationMinutes: 40 for new patients, 30 for routine visits, 20 for follow-ups**
- NEVER suggest times without checking first - always verify availability before offering options
- Example workflow for NEW PATIENT (40 minute appointment):
  1. Patient says "I want to come on November 13th"
  2. You call: check_doctor_availability with startDate="2025-11-13", endDate="2025-11-13", durationMinutes: 40
  3. Backend returns ALL available slots for that day with 40-minute intervals
  4. You show patient: "Here are the available times on November 13th: 9:00 AM, 9:40 AM, 10:20 AM, 11:00 AM, 11:40 AM, ..."
  5. Patient picks a time (e.g., "9:00 AM is perfect")
- **NEVER manually check individual times - let the backend do it by fetching all slots at once**
- If patient requests a specific day, call check_doctor_availability for that entire day with correct durationMinutes
...
- Share pre-visit notes: "Here's what you should prepare: [pre-visit instructions]"
- Be warm and encouraging: "These preparations will help us make the most of your visit!"
- Friendly close
...
2. **EXTRACT THE APPOINTMENT DURATION FROM PATIENT CONTEXT:**
   - Find the "=== UPCOMING APPOINTMENTS ===" section
   - Look for the appointment matching the date/time mentioned in the patient context
   - Extract the duration in parentheses, e.g., "(40 minutes)" or "(30 minutes)"
   - Example: "Type: new_patient (40 minutes)" → Use 40 for durationMinutes parameter
3. Ask when they'd prefer: "What date and time would work better for you?" or "Which day works best?"
4. Wait for their response (patient may say just a day, or a day with time)

**CRITICAL: BEFORE CALLING check_doctor_availability - ALWAYS EXTRACT DURATION:**
⚠️ **YOU MUST DO THIS EVERY TIME:**
1. Look at "=== UPCOMING APPOINTMENTS ===" section in patient context
2. Find the appointment TYPE line that shows: "Type: [type] ([number] minutes)"
3. Extract the NUMBER from the parentheses - this is your durationMinutes
4. Examples:
   - "Type: new_patient (40 minutes)" → durationMinutes: 40
   - "Type: urgent (20 minutes)" → durationMinutes: 20
   - "Type: follow_up (30 minutes)" → durationMinutes: 30
5. **NEVER use default 30 minutes** - ALWAYS use the extracted number from the appointment type
6. Pass this value as durationMinutes parameter in check_doctor_availability call

**Once patient provides a date/day:**
5. **IMMEDIATELY fetch all available slots for that day** using check_doctor_availability:
   ...
   - **CRITICAL: durationMinutes = the duration you extracted ABOVE** (NOT a default value!)
   - Example: If appointment is "Type: new_patient (40 minutes)", then pass durationMinutes: 40
   - This ensures the backend checks for conflicts with the correct appointment length
   - Available slots are generated based on this duration (40-minute intervals for 40-minute appointments)
   - This checks doctor's Google Calendar and shows real available slots

6. **RESPOND BASED ON WHAT PATIENT ASKED FOR:**
   - **If patient asked: "What days are available?" or "Which days are available?" or similar:**
     - ONLY respond with day names: "Monday, Tuesday, Wednesday, Thursday, and Friday are all available!"
     - NEVER include time slots in your response
     - Then ask: "What time would work best for you on one of those days?"
     - Example of WRONG: "Monday: 12 PM, 1 PM, 2 PM..." ❌
     - Example of RIGHT: "Monday, Tuesday, Wednesday, Thursday, and Friday are available. What time works for you?" ✓

   - **If patient asked: "What times are available on Wednesday?" or "What slots on Wednesday?" or similar:**
     - Then show the times: "Here are the available times on Wednesday: 9:00 AM, 9:30 AM, 10:00 AM, 10:30 AM..."
     - Show ALL available times for that day

   - **If their suggested time is in the list, highlight it:** "Great! 3 PM is available!"
   - **CRITICAL: Do NOT mix format - either show ONLY days OR show times, never both unless patient asks for both**

7. **Wait for patient confirmation:**
...
8. IMPORTANT: When the tool returns with appointment AND doctorInstructions:
   - **ALWAYS include the doctor's instructions in your confirmation message**
   - Share the AI instructions: "Before your appointment, Dr. [Name] recommends: [instructions]"
   - Share the pre-visit notes: "Here's what you should prepare: [pre-visit instructions]"
   - Be warm and encouraging: "These preparations will help us make the most of your visit!"
   - Emphasize any important preparation needed
   - This is a critical part of the patient experience - don't skip it!
...
- YOU: "Perfect! I've rescheduled your appointment to Wednesday at 3 PM. Before your appointment, Dr. [Name] recommends: [share the doctor's AI instructions and pre-visit notes]. These preparations will help us make the most of your visit!"
...
async function getDoctorAvailableSlots(
  ...
  // Ensure durationMinutes is valid - minimum 20 minutes
  if (!durationMinutes || durationMinutes < 20) {
    console.log(`⚠️ SLOT GENERATION: durationMinutes was ${durationMinutes}, defaulting to 40`);
    durationMinutes = 40;
  }

  console.log(`\n🎯 SLOT GENERATION STARTED`);
  console.log(`   📍 Duration for slot intervals: ${durationMinutes} minutes`);
  ...
  console.log(`\n📋 CHECKING AVAILABILITY (9 AM - 4 PM, Monday-Friday):\n`);
  ...
  while (current <= endDate) {
    ...
    if (dayOfWeek >= 1 && dayOfWeek <= 5) {
      ...
      // 9 AM to 4 PM Beirut time (hour < 16 means up to 3:59 PM, ensuring slot ends by 4 PM)
      for (let hour = 9; hour < 16; hour++) {
        for (let minute = 0; minute < 60; minute += durationMinutes) {
          // Log the minute increment to verify it's using durationMinutes correctly
          if (minute === 0) {
            console.log(`        Minute increment: +${durationMinutes} minutes`);
          }
          ...
          // Check if slot ends after clinic closing time (4 PM Beirut = 14:00 UTC)
          // Clinic closes at 16:00 Beirut = 14:00 UTC
          const clinicCloseUTC = new Date(Date.UTC(
            current.getFullYear(),
            current.getMonth(),
            current.getDate(),
            14, // 14:00 UTC = 16:00 Beirut
            0,
            0,
            0
          ));
          if (slotEnd > clinicCloseUTC) {
            continue; // Skip slots that end after clinic hours
          }
  ...
  try {
    ...
    if (upcomingVisits.length > 0) {
      upcomingVisits.forEach((visit) => {
        // Format date as text (e.g., "Monday, November 25, 2024 at 2:30 PM") to avoid DOB redaction
      ...
      context += `[NO UPCOMING APPOINTMENTS] - Patient has no scheduled appointments at this time\n`;
    }
    context += `\n`;
...
export async function generateChatResponse(
  ...
  try {
    ...
    // Redact PII from patient context string before sending to AI
    // Note: getPatientContext returns a string, not an object
    const redactedContext = redactPII(patientContext).redactedText;
...
⚠️ **CRITICAL - YOU MUST INCLUDE durationMinutes PARAMETER!**
The durationMinutes MUST match the appointment type from "=== UPCOMING APPOINTMENTS ===" section:
- If appointment says "Type: new_patient (40 minutes)" → durationMinutes: 40
- If appointment says "Type: urgent (20 minutes)" → durationMinutes: 20  
- If appointment says "Type: follow_up (30 minutes)" → durationMinutes: 30
- ALWAYS extract the NUMBER from parentheses and pass it as durationMinutes
- DO NOT use default 30 - the duration MUST match the actual appointment type
...
**USE CASE 1: RESCHEDULING EXISTING APPOINTMENTS**
When a patient wants to reschedule and provides a date/day:
1. Patient says: "I'd like Wednesday at 3 PM" or just "Wednesday"
2. EXTRACT durationMinutes from their appointment type (look for "([number] minutes)" in patient context)
3. YOU: Call check_doctor_availability with correct durationMinutes
4. YOU: Display all available slots to patient: "Here are available times on Wednesday: 10 AM, 11 AM, 12 PM, 2 PM, 3 PM, 4 PM"
5. Patient: Confirms or picks from the list
6. YOU: Reschedule with their confirmed time

**USE CASE 2: CHECKING AVAILABILITY FOR NEW APPOINTMENTS**
When a patient asks "what times are available on [day]?" (without context of existing appointment):
1. **DEFAULT to durationMinutes: 40** (standard new patient consultation duration)
2. Call check_doctor_availability with startDate=[requested_day], endDate=[requested_day], durationMinutes: 40
3. Display: "Here are available times on [day] for a new patient appointment: 9:00 AM, 9:40 AM, 10:20 AM, 11:00 AM, ..."
4. Let patient pick from list
    ...
    if (responseMessage?.tool_calls && responseMessage.tool_calls.length > 0) {
      ...
      if (allCheckAvailability && toolCalls.length > 1) {
        ...
        if (functionCall?.name === 'check_availability') {
          ...
          // CRITICAL FIX: Override the legacy 30-minute default to 40 minutes for new patient appointments
          // The standard appointment duration for initial consultations is 40 minutes
          // If AI passes 30 or less (or doesn't pass anything), override to 40
          if (!args.durationMinutes || args.durationMinutes <= 30) {
            console.log(`⚠️ OVERRIDE: durationMinutes was ${args.durationMinutes || 'not set'}, setting to 40 minutes for appointment booking`);
            args.durationMinutes = 40;
          }
          ...
          try {
            ...
            // Get doctor instructions to share with patient
            ...
            const followUpMessages: OpenAI.Chat.ChatCompletionMessageParam[] = [
              ...
              responseMessage,
              {
                ...
                content: JSON.stringify({
                  ...
                  doctorInstructions: instructionsResult.success ? instructionsResult.instructions : null,
                }),
              },
            ];
          ...
          // Log what durationMinutes was provided
          console.log(`🔍 CHECK_DOCTOR_AVAILABILITY called with: durationMinutes=${args.durationMinutes}, startDate=${args.startDate}, endDate=${args.endDate}`);

          // CRITICAL FIX: Override the legacy 30-minute default to 40 minutes for new patient appointments
          // The standard appointment duration for initial consultations is 40 minutes
          // If AI passes 30 or less (or doesn't pass anything), override to 40
          if (!args.durationMinutes || args.durationMinutes <= 30) {
            console.log(`⚠️ OVERRIDE: durationMinutes was ${args.durationMinutes || 'not set'}, setting to 40 minutes for new patient appointment`);
            args.durationMinutes = 40;
          }
